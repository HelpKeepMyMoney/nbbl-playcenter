...
- **Allnet demo & branding:** Hub header title uses **NBBL** as a wordmark (**NB** white / **BL** red), **ALLNET** (white), and yellow **(Demo)**. Theme accent is **logo red** (Tailwind `red-*`, `--primary` / `--ring` in `src/index.css`); **yellow** is used for key highlights (hero **inspire**, **Admin** button, **Add to Content Hub** in the hero bullet list, `.selection-nbbl` for text selection). Sign-in title uses the same **NB** / **BL** split plus **PlayCenter**. Placeholder thumbnail (`src/lib/thumbnail.ts`) renders **NB** / **BL** in white / red before “ PlayCenter”.
...
- **Admin panel:** filter **Pending / Live / Denied / Private / All**; **Approve** or **Deny** (denial **requires a reason** shown to the player). Loads **`users/{ownerUid}`** for display name + email when available. **Migrate legacy public** fixes old clips that used `isPublic` before the moderation model. Admins can **delete any clip** from the player (see changelog).
...
| `src/components/ContentHub.tsx` | Hero (lead + **bullets** per tab), **My clips / Content Hub** feed toggle, status **banner**, filters + search, **Allnet / NBBL** header + **footer** (NBBL logo + copy), yellow **Admin** button when applicable, bottom nav; **marketing links** on header/footer logos |
...
| `firestore.rules` | `users`, `admins`, `clips` (owner, published **Content Hub** read model, admin reads/updates; **admin** may delete clips and like subdocs) |
