...
- **Hub (`ContentHub`):** NBBL logo in the **footer**; logos in **header** and **footer** link to [nbbl.vercel.app](https://nbbl.vercel.app/) (new tab). **Community-status orange banner** stays until the user taps **×** (no auto-dismiss timer in `App.tsx`).
- **Recorder / preview:** Blob URL on the trim preview is attached in **`useLayoutEffect`** (with rAF fallback if the ref is late). Each new capture or upload bumps **`previewGeneration`** so the preview `<video>` **remounts** with a fresh `key`. **`preload="metadata"`** and native **controls**. **`isLikelyVideoFile`** accepts some picks with empty or generic MIME when the extension looks like video. If the browser cannot decode (e.g. **HEVC** on Windows), an **amber note** explains re-exporting as **H.264** / “Most compatible” (uses **`TRANSCODE_UNSUPPORTED_HINT`** from `videoProcess.ts` when the element errors).
- **Trim / transcode (`videoProcess.ts`):** Trim re-encode uses **`canvas.captureStream`** + **`drawImage`** each frame (max long edge **1280px**) + **MediaRecorder**, instead of **`HTMLVideoElement.captureStream`** — better for **phone MP4/MOV** and fewer **“Encoding failed”** cases. **VP8** mime variants are **preferred before VP9**. Waits for **non-zero `videoWidth`/`videoHeight`** (including a short **play** probe); if dimensions never appear, throws **`TRANSCODE_UNSUPPORTED_HINT`**. **`createMediaRecorder`** fallbacks, **`requestData()`** before **`stop()`**. Re-encoded clips are **video-only / silent**; **`canSkipCameraTranscode`** full-length camera path still keeps **audio**.
...
- **Recorder:** **Camera** (`MediaRecorder`, **60s** hard limit, countdown, front/rear switch) **or** **From camera roll** (`<input type="file" accept="video/*">`). After capture or pick, **trim** the clip with **start/end sliders** before save (still capped at 60s of output). **Preview** uses `useLayoutEffect` for the blob URL, **`previewGeneration`** + **`key`** to remount per clip, **`preload="metadata"`**, and an optional **amber decode hint** when the preview never gets picture dimensions or hits a media **error**. Picks with **empty or generic MIME** types are accepted when the filename looks like a video (e.g. some Android exports). **HEVC** on many **Windows** browsers still won’t preview or trim — re-export as **H.264** / “Most compatible”.
- **Upload processing (`src/lib/videoProcess.ts`):** Clips are re-encoded in the browser so the uploaded file stays **≤ 20 MB** (bitrate is stepped down until the cap is met). The trim path **plays** the source, draws frames to a **canvas** (scaled), uses **`canvas.captureStream`** + **WebM**-family **MediaRecorder** (**VP8-first** mime selection). **No audio** on the re-encode path. A **fast path** skips re-encode for short **full-length** camera recordings already under the cap (**audio preserved**). **Uploaded `durationSec`** reflects the **trimmed** segment. **`TRANSCODE_UNSUPPORTED_HINT`** documents the HEVC / desktop limitation when decode never succeeds.
...
- **Hub layout:** **Hero** first, then **My clips / Community**, then a **dismissible orange banner** when a **your** clip’s **community status** changes (e.g. submitted for review, approved, denied with reason, withdrawn). The banner **stays until the user dismisses** it (×). Below that: errors, **category filters + search**, then the grid.
...
| `src/components/Recorder.tsx` | Camera **or** file pick, **trim** sliders, preview blob URL + **previewGeneration** + decode **note** banner, transcode path, **Request Community** on save |
...
| `src/lib/videoProcess.ts` | **≤20 MB** transcode, trim via **play → canvas + `captureStream` → MediaRecorder** (scaled), VP8-first mime, dimension probe + **`TRANSCODE_UNSUPPORTED_HINT`**, `requestData` before `stop`, duration probe, `canSkipCameraTranscode` |
