...
| Auth & data | [Firebase](https://firebase.google.com/) Auth (**Google** + **email/password**), [Cloud Firestore](https://firebase.google.com/docs/firestore), [Cloud Storage](https://firebase.google.com/docs/storage), [Cloud Functions](https://firebase.google.com/docs/functions) (callables: account deletion + admin role; Firestore trigger: **Resend** email when a clip enters **pending** review) |
| Marketing CRM | [Constant Contact](https://www.constantcontact.com/) via **Vercel** serverless [`api/constant-contact-sync.js`](api/constant-contact-sync.js) (not Cloud Functions). Opt-in at **email sign-up** only; server verifies Firebase **ID token** and Firestore **`marketingConsent`** before calling CC’s **`sign_up_form`** API. |
| Hosting | [Vercel](https://vercel.com/) (`vercel.json`: SPA rewrite **excluding** `/api/*` so serverless routes work) |
...
- **Marketing consent + Constant Contact (Vercel):** Email **Sign up** includes an optional **marketing** checkbox (unchecked by default). On first Firestore profile write, **`marketingConsent`** is stored on **`users/{uid}`** (`true` only if checked; **`false`** for Google-first accounts and unchecked sign-ups). **`upsertUserProfileFromAuth`** accepts optional **`{ marketingConsent }`** and **only sets that field when the profile document is first created**, so later sign-ins do not overwrite consent. **Firestore rules** require **`marketingConsent`** (bool) on **create** and validate it on updates when present. After a successful **email sign-up** with the box checked, the client calls **`POST /api/constant-contact-sync`** with a fresh **Firebase ID token** ([`src/lib/constantContactSync.ts`](src/lib/constantContactSync.ts)). The **Vercel** function verifies the token, re-reads **`users/{uid}`** (must have **`marketingConsent === true`**), then syncs name + email to Constant Contact using the same OAuth + **`sign_up_form`** flow as the main NBBL site’s **`api/contact.js`**. **Environment (Vercel or root `.env` for `vercel dev`):** **`CONSTANT_CONTACT_CLIENT_ID`**, **`CONSTANT_CONTACT_CLIENT_SECRET`**, **`CONSTANT_CONTACT_REFRESH_TOKEN`**, **`CONSTANT_CONTACT_LIST_ID`** or **`CONSTANT_CONTACT_LIST_IDS`**, and **`FIREBASE_SERVICE_ACCOUNT_KEY`** (single-line JSON for a GCP service account with Firestore access to read/update **`users/{uid}`**). **Do not** prefix these with **`VITE_`** (that would expose secrets to the browser). **`VITE_APP_ORIGIN`** (optional, no trailing slash) is only needed if the SPA is served from a **different origin** than the API. **Cloud Functions** no longer include Constant Contact (removed **`syncConstantContactOnUserCreate`**); deploy **`firebase deploy --only functions`** to drop the old function if it was ever deployed. Local **`npm run dev`** (Vite only) does **not** serve **`/api/*`** — use **`vercel dev`** to exercise the sync locally with **`.env`** in the repo root.
...
- **Cloud Functions (`functions/`):** **`deleteUserAccount`** and **`setUserAdminRole`** are **callable** HTTPS functions (2nd gen, **`firebase-functions` v2**). They share **`ADMIN_CALLABLE_BASE`** in **`functions/index.js`**: **`invoker: 'public'`** so browser **OPTIONS** preflight reaches the function on **Cloud Run** (handlers still require Firebase Auth + **`admins/{callerUid}`**), and **`cors: true`** (any web origin; same as Firebase callable default). If the admin UI still hits **CORS blocked**, check the failing URL in DevTools **Network**: it must be **`us-central1-nbbl-playcenter`** ( **`nbbl`** with a letter **l** ). If you see **`nbb1-playcenter`** (digit **1**), **`VITE_FIREBASE_PROJECT_ID`** is wrong in that build’s env (e.g. Vercel) — fix it and redeploy the site; **`vite.config.ts`** also **fails the build** locally if that typo is present. Otherwise redeploy functions and confirm **`invoker: 'public'`** is present (needed for Cloud Run **OPTIONS** preflight). Root **`npm run deploy:functions`** runs `firebase deploy --only functions`. On **PowerShell**, quote combined deploy targets, e.g. `firebase deploy --only "functions,firestore:rules"`. First-time deploy may require enabling **Cloud Build**, **Artifact Registry**, and related APIs; if the build fails on **Artifact Registry** permissions, grant the **Google Cloud Functions service agent** (**`service-PROJECT_NUMBER@gcf-admin-robot.iam.gserviceaccount.com`**) **Artifact Registry Reader** on the project (see [Cloud Functions troubleshooting](https://cloud.google.com/functions/docs/troubleshooting#build-service-account)). After deploy, you can run **`firebase functions:artifacts:setpolicy`** to add a container image cleanup policy in **`us-central1`**. **`admin:set-claim`** remains optional; **`setUserAdminRole`** merges the **`admin`** claim when promoting and removes it when demoting (users may need to **sign out and back in** for Storage to see the new token).
- **Admin review email ([Resend](https://resend.com/)):** Firestore trigger **`notifyAdminClipPendingReview`** runs when a **`clips/{id}`** document’s **`communityVisibility`** **transitions to** **`pending`** (e.g. new upload with **Add to Content Hub**, or an owner moving a clip back into review). It emails **nobackboard@gmail.com** with clip title, owner label, owner uid, and clip id. Sending uses the **`resend`** npm package and the **`RESEND_API_KEY`** [Secret Manager](https://firebase.google.com/docs/functions/config-env#secret-manager) secret on the function (use the same API key as the main NBBL site’s contact form). **`firebase functions:secrets:set RESEND_API_KEY`** (stdin or **`--data-file`**) then **`npm run deploy:functions`**; the deploy grants the runtime service account **secret accessor** on that secret. **From** address is fixed in code to **`NBBL Website <contact@thecpi.network>`** (same verified sender pattern as NBBL’s **`api/contact.js`**). The first **Gen2 Firestore** trigger in a project can briefly fail with **Eventarc Service Agent** permissions — wait a minute or two and **redeploy** if create errors with that message.
...
- **Sign-in:** Google (popup) or **email/password** with **Sign in** / **Sign up** on the same screen; clips and files scoped to `request.auth.uid`. Title line matches the hub: **NBBL ALLNET (Demo)** (same colors as the header). **Sign up** shows an optional **email me updates** checkbox; see **Marketing consent + Constant Contact** in the changelog.
...
- `displayName`, `email`, `photoURL` (nullable), **`city`** (optional string), **`marketingConsent`** (bool, set **once** on first profile create), `createdAt`, `updatedAt`
- After a successful Constant Contact sync (Vercel API), the server may add **`constantContactSyncedAt`**, **`constantContactSyncError`**, and **`constantContactContactId`** (clients do not write these directly for sync; Admin SDK in **`api/constant-contact-sync.js`** updates them)
...
| `src/components/SignInScreen.tsx` | Email/password + Google; **NB/BL/ALLNET/(Demo)** title + **Basketball remixed**; **awaits** `users/{uid}` upsert after successful sign-in/sign-up; **sign-up** marketing checkbox + **`requestConstantContactSync`** when opted in |
...
| `src/lib/userProfile.ts` | **`upsertUserProfileFromAuth`**, **`UpsertUserProfileOptions`** / **`marketingConsent` on first create only**, **`fetchUserProfilesByIds`** (admin), **`subscribeToUsersForAdmin`**, **`adminUpdateUserProfileFirestore`** |
| `src/lib/constantContactSync.ts` | **`requestConstantContactSync`** — `POST` Vercel **`/api/constant-contact-sync`** with Firebase **ID token** |
| `api/constant-contact-sync.js` | **Vercel** Node serverless: verify ID token, check Firestore **`marketingConsent`**, OAuth + CC **`sign_up_form`**, optional user-doc sync status fields |
...
| `firestore.rules` | `users` (**`marketingConsent`** required on create; bool validation when present), `admins` (moderators may **read** all `admins/*`; client **writes** disabled), `clips` (owner, published **Content Hub** read model, admin reads/updates; **admin** may delete clips and like subdocs) |
...
| `scripts/grant-admin-callable-invoker.mjs` | **gcloud:** grant **Cloud Run Invoker** to **allUsers** on admin callable Run services (see **Firebase deploy**) |
...
| `vite.config.ts` | Vite + React + Tailwind; **build fails** if **`VITE_FIREBASE_PROJECT_ID`** is the typo **`nbb1-playcenter`** instead of **`nbbl-playcenter`** |
| `vercel.json` | SPA fallback to **`index.html`** for paths that **do not** start with **`api/`** (so **`/api/constant-contact-sync`** hits the serverless handler) |
| `functions/index.js` | Callable **`deleteUserAccount`**, **`setUserAdminRole`**; Firestore **`notifyAdminClipPendingReview`** (Admin SDK + **Resend**; **`ADMIN_CALLABLE_BASE`**: **`us-central1`**, **`invoker: 'public'`**, **`cors: true`**) — **no** Constant Contact in Functions |
| `functions/package.json` | Cloud Functions dependencies (`firebase-admin`, `firebase-functions`, **`resend`**) |
...
4. Copy `.env.example` to **`.env.local`** and set all `VITE_FIREBASE_*` variables (Project settings → Your apps → SDK config). **`VITE_FIREBASE_PROJECT_ID`** must be **`nbbl-playcenter`** (letter **l** in **`nbbl`**, not digit **`1`**). If you deploy Functions outside **`us-central1`**, set **`VITE_FIREBASE_FUNCTIONS_REGION`** to the same region.
5. Deploy **Firestore rules + indexes** and **Storage rules** ([Firebase deploy](#firebase-deploy)). For **admin account deletion, role changes, and review-notification email**, install dependencies in **`functions/`** (`npm install`), enable **Blaze**, set the **`RESEND_API_KEY`** function secret (see **Admin review email** in the changelog and `.env.example`), and deploy **Functions** (`npm run deploy:functions` or `firebase deploy --only functions`).
...
**Functions:** Run **`npm install`** inside **`functions/`** before the first deploy. **`notifyAdminClipPendingReview`** needs **`firebase functions:secrets:set RESEND_API_KEY`** (same key as the NBBL Resend project is fine); without it the function logs a warning and skips email. If deploy fails on **Artifact Registry** access, fix **IAM** for the **Cloud Functions service agent** (see changelog above) and retry. Optional: **`firebase functions:artifacts:setpolicy`** to configure image cleanup in **`us-central1`**.

**Admin callables (`setUserAdminRole`, `deleteUserAccount`) — “CORS” on localhost or production:** If DevTools shows *No `Access-Control-Allow-Origin` header* on the **OPTIONS** preflight, check the response **status**. A **403** HTML page (“Your client does not have permission to get URL …”) means **Cloud Run IAM**, not your `cors` option: unauthenticated **OPTIONS** must be allowed so the Firebase runtime can answer the preflight. After deploy, if it still fails:

1. **One command (needs [gcloud](https://cloud.google.com/sdk), project access):** from the repo root, run **`npm run grant:callable-invoker`** — it grants **`roles/run.invoker`** to **`allUsers`** on Cloud Run services whose names match those callables in **`us-central1`** (override project with **`GCLOUD_PROJECT`**, region with **`FUNCTIONS_REGION`** if needed). This does **not** weaken Firebase Auth inside the function; it only opens the HTTPS front door so browsers can preflight and POST with a Bearer ID token.
2. **Console:** [Cloud Run](https://console.cloud.google.com/run) → select the service for each function → **Security** → **Allow public access** (same IAM as above).

**Sanity check (403 vs real CORS):** `curl.exe -i -X OPTIONS "https://us-central1-<PROJECT>.cloudfunctions.net/setUserAdminRole" -H "Origin: http://localhost:3000" -H "Access-Control-Request-Method: POST" -H "Access-Control-Request-Headers: authorization,content-type"` — expect **204** (or **200**) with `access-control-allow-origin` once Invoker is fixed; **403** means fix IAM first.
...
3. For **Constant Contact sync** (see changelog **Marketing consent + Constant Contact**), add **server-only** variables in Vercel (**Settings → Environment Variables**), same names as in [`.env.example`](.env.example): **`CONSTANT_CONTACT_CLIENT_ID`**, **`CONSTANT_CONTACT_CLIENT_SECRET`**, **`CONSTANT_CONTACT_REFRESH_TOKEN`**, **`CONSTANT_CONTACT_LIST_ID`** or **`CONSTANT_CONTACT_LIST_IDS`**, **`FIREBASE_SERVICE_ACCOUNT_KEY`**. Optionally **`VITE_APP_ORIGIN`** if the static app is not same-origin as the deployment that serves **`/api/*`**.
4. Firebase **Authentication → Authorized domains:** add your Vercel host.
5. Apply **Storage CORS** on the default bucket for production.

The **`.vercel`** directory (CLI link metadata) is **gitignored** so it is not committed.

**Local full stack (optional):** Run **`vercel dev`** from the repo root so **`/api/constant-contact-sync`** runs; put secrets in root **`.env`** (gitignored). Plain **`npm run dev`** (Vite) does not expose **`/api/*`**.
...
| `npm run grant:callable-invoker` | **gcloud:** grant **Cloud Run Invoker** to **allUsers** on admin callable services (fixes OPTIONS **403** “CORS”); see **Firebase deploy** |
