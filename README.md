...
- **Trim / transcode (`videoProcess.ts`):** Trim re-encode uses **`canvas.captureStream`** + **`drawImage`** each frame (max long edge **1280px**) + **MediaRecorder**. **Video-only** WebM mimes are tried **before** `vp8,opus` / `vp9,opus` so Chrome does not fail the muxer when there is **no audio track**. **`seekTo`** skips work when already at the target, and uses an **8s timeout** if **`seeked`** never fires (fixes hangs at trim start **0**). One **`drawImage`** before **`MediaRecorder.start()`** primes the canvas track. Waits for **non-zero dimensions** (with a **play** probe); **`TRANSCODE_UNSUPPORTED_HINT`** when decode never succeeds. **`createMediaRecorder`** fallbacks, **`requestData()`** before **`stop()`**. Re-encoded clips are **silent**; **`canSkipCameraTranscode`** camera path keeps **audio**.
...
- **Upload processing (`src/lib/videoProcess.ts`):** Clips are re-encoded in the browser so the uploaded file stays **≤ 20 MB** (bitrate is stepped down until the cap is met). The trim path **plays** the source, draws to a **canvas** (scaled), **`canvas.captureStream`** + **WebM** **MediaRecorder** with **`video/webm;codecs=vp8`** (etc.) **before** opus-including mimes. **`seekTo`** is timeout-safe. **No audio** on the re-encode path. **Fast path** skips re-encode for qualifying **full-length** camera clips (**audio preserved**). **`TRANSCODE_UNSUPPORTED_HINT`** for undecodable HEVC-style sources on some desktops.
...
| `src/lib/videoProcess.ts` | **≤20 MB** transcode, trim via **canvas + `captureStream` → MediaRecorder** (scaled), **video-only WebM mimes before opus**, **robust `seekTo`** (timeout + already-at-target), priming `drawImage`, **`TRANSCODE_UNSUPPORTED_HINT`**, `requestData` before `stop`, `canSkipCameraTranscode` |
