...
- Client-generated **JPEG thumbnails** uploaded with each video ( **`captureThumbnailFromVideoBlob`** or **`generatePlaceholderThumbnail`** when video can’t be decoded — neutral **NBBL PlayCenter** play-button artwork for hub grids, not browser-specific copy); Storage filenames use **`video.webm`** or **`video.mp4`** depending on the output blob type (`src/lib/clips.ts`).
