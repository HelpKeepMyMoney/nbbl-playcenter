...
- **Recorder / preview:** Blob URL on the trim preview is attached in **`useLayoutEffect`** (with rAF fallback if the ref is late). Each new capture or upload bumps **`previewGeneration`** so the preview `<video>` **remounts** with a fresh `key` (avoids stale decoder state). Preview uses **`preload="metadata"`** and native **controls** (no forced programmatic play/pause loop). **`isLikelyVideoFile`** still accepts some picks with empty or generic MIME when the extension looks like video.
- **Trim / transcode (`videoProcess.ts`):** **`recordSegmentToBlob`** calls **`video.play()` before `MediaRecorder.start()`** (with **`muted`** so autoplay policies allow it), records from a **video-only** `MediaStream` from `captureStream` (avoids many **“Recording failed”** encoder errors from mismatched audio). **`video/mp4`** is not used as a MediaRecorder mime candidate here. **`createMediaRecorder`** falls back through mime+bitrate → mime only → bitrate only → default recorder; **`requestData()`** before **`stop()`** helps flush the last chunk; clearer errors if the encoder fails or the output is empty. **Note:** clips that go through this re-encode path are **silent** (audio is stripped); **full-length in-app** recordings that hit **`canSkipCameraTranscode`** still keep audio.
...
- **Recorder:** **Camera** (`MediaRecorder`, **60s** hard limit, countdown, front/rear switch) **or** **From camera roll** (`<input type="file" accept="video/*">`). After capture or pick, **trim** the clip with **start/end sliders** before save (still capped at 60s of output). **Preview** uses `useLayoutEffect` to set the blob URL once the preview `<video>` exists, **`previewGeneration`** + **`key`** to remount per clip, and **`preload="metadata"`** with native controls. Picks with **empty or generic MIME** types are accepted when the filename looks like a video (e.g. some Android exports). **HEVC / “High Efficiency”** files may still not decode in some desktop browsers; re-export as **H.264** if preview or trim fails.
- **Upload processing (`src/lib/videoProcess.ts`):** Clips are re-encoded in the browser (video element + `captureStream` + `MediaRecorder`) so the uploaded file stays **≤ 20 MB** (bitrate is stepped down until the cap is met). The trim pipeline **plays the source before recording**, uses **WebM**-family mime types only for the recorder, and records **video tracks only** for stability (**no audio** on that path). A **fast path** skips re-encode for short **full-length** camera recordings that are already under the cap (**audio preserved** there). **Uploaded `durationSec`** reflects the **trimmed** segment; camera wall-clock time is still used when metadata is unreliable. Re-encoding support **varies by browser** (test camera-roll uploads on target devices).
...
| `src/components/Recorder.tsx` | Camera **or** file pick, **trim** sliders, preview blob URL + **previewGeneration** remount key, transcode path, **Request Community** on save |
...
| `src/lib/videoProcess.ts` | **≤20 MB** transcode, trim via **play → captureStream (video-only) → MediaRecorder**, mime fallbacks, `requestData` before `stop`, duration probe, `canSkipCameraTranscode` |
