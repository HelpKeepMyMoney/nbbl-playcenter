...
import { generateTournamentSchedule, type TeamInfo } from "../tournaments/lib/generate";
import {
  computeStandings,
  generateDemoScores,
  type MatchResult,
...
import { simulateTournamentMatches } from "../tournaments/lib/simulate-tournament";
import {
  simulatePlayerStats,
  loadPlayersByTeam,
...
import { resetTeamGameStats, simulateTeamStats } from "../tournaments/lib/simulate-team-stats";
import { syncPlayerStatsToSeedFile } from "../tournaments/lib/player-stats-seed-export";
import { syncTeamStatsToSeedFile } from "../tournaments/lib/team-stats-seed-export";
...
async function buildSeedData(tournamentId: string): Promise<SeedTournamentData> {
  ...
  if (!tSnap.exists) throw notFound("Tournament not found");
  const t = tSnap.data()!;
...
export async function handleCreateTournamentDraft(ctx: AuthContext, data: unknown) {
requirePermission(ctx, PERMISSIONS.TOURNAMENTS_WRITE);

  const input = createTournamentDraftSchema.parse(data);
...
export async function handleSaveTournament(ctx: AuthContext, data: unknown) {
requirePermission(ctx, PERMISSIONS.TOURNAMENTS_WRITE);

  const { tournamentId } = tournamentIdSchema.parse(data);
  ...
  if (doc.tenantId !== ctx.tenantId) {
    throw permissionDenied("Tournament not in your tenant");
  }
...
export async function handleRecalculateTournamentSchedule(ctx: AuthContext, data: unknown) {
requirePermission(ctx, PERMISSIONS.TOURNAMENTS_WRITE);
...
export async function handleUpdateTournament(ctx: AuthContext, data: unknown) {
requirePermission(ctx, PERMISSIONS.TOURNAMENTS_WRITE);

  const input = updateTournamentSchema.parse(data);
...
export async function handleDeleteTournament(ctx: AuthContext, data: unknown) {
requirePermission(ctx, PERMISSIONS.TOURNAMENTS_WRITE);
...
export async function handleRecordMatchResult(ctx: AuthContext, data: unknown) {
requirePermission(ctx, PERMISSIONS.TOURNAMENTS_WRITE);

  const input = recordMatchResultSchema.parse(data);
  const matchRef = getDb().collection("tournamentMatches").doc(input.matchId);
  const matchSnap = await matchRef.get();

  if (!matchSnap.exists) throw notFound("Match not found");
...
export async function handleSimulateTournament(ctx: AuthContext, data: unknown) {
requirePermission(ctx, PERMISSIONS.TOURNAMENTS_WRITE);
